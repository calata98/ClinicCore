...
ClinicCore needs clear domain boundaries without the operational cost of microservices. The MVP includes identity, clinic management, patients, scheduling, treatment notes, packages, exercise plans, analytics, and audit logs.
...
The database model stores foreign keys as IDs between modules. This keeps module boundaries explicit and avoids leaking JPA relationships into API workflows.
...
- A future split into services is possible, but it is not part of the initial plan.
